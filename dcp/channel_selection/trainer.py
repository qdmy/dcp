...
class SegmentWiseTrainer(object):
    ...
    def create_segments(self):
        ...
        self.logger.info("init shallow head done!") # only show in this line

        # the second time code came to this place, pruned model is wrong
        # print('-----------check pruned-----------')
        # print(type(self.pruned_model))
        ...
        if self.settings.net_type in ["resnet", "preresnet"]:
            for ori_module, pruned_module in zip(self.original_model.modules(), self.pruned_model.modules()):
                if isinstance(ori_module, (PreBasicBlock, Bottleneck, BasicBlock)):
                    ...
                    block_count += 1
                    print('****************DEBUG*****************')
                    print("pruned enter block: {}".format(type(pruned_module)))
                    # print('ori_module: \n{}'.format(ori_module))
                    print('{}th, pruned_module: \n{}'.format(block_count, pruned_module))
                    print('****************DONE******************')

                    # if block_count is equals to pivot_num, then create new segment
                    if block_count in self.settings.pivot_set:
                        print('------this segment is over------------')
    ...
    def create_auxiliary_classifiers(self):
        ...
        for i in range(len(self.pruned_segments) - 1):
            ...
            # liuxu adds for the first layer of preresnet20
            # elif isinstance(self.pruned_segments[i][-1], (nn.Conv2d, nn.BatchNorm2d, nn.ReLU)):
            #     # in_channels = self.pruned_segments[i][-1].out_channels
            #     print('next layer in_channel is the first layer out_channel: {}'.format(in_channels))
            #     continue
            else:
                print(i, type(self.pruned_segments[i][-1]), self.pruned_segments[i][-1])
    ...
    def insert_additional_losses(self):
        ...
        self.create_auxiliary_classifiers() # problem happened
    ...
    def update_model(self, original_model, pruned_model, aux_fc_state=None, aux_fc_opt_state=None, seg_opt_state=None):
        ...
        # print('-----------check pruned foremove 1-----------')
        # print(type(self.pruned_model))
        # print(self.pruned_model)
        # print('=====================================')

        # print('----------what happened to pruned model--------')
        # for ori_module, pruned_module in zip(self.original_model.modules(), self.pruned_model.modules()):
        #     print('*****************DEBUG****************')
        #     # print('ori_module: \n{}'.format(ori_module))
        #     print('pruned_module: \n{}'.format(pruned_module))
        #     print('*****************DONE*****************')
        self.insert_additional_losses() # problem happened
