...
from dcp.channel_selection.channel_selection import LayerChannelSelection
...
from dcp.channel_selection.dcp_checkpoint import DCPCheckPoint
from dcp.channel_selection.option import Option
from dcp.channel_selection.trainer import SegmentWiseTrainer
...
class Experiment(object):
    ...
    def __init__(self, options=None, conf_path=None):
        ...
        self.pruned_model = None # this model contains some attribute that can tell which channel of each layer would be selected, so does how mang channels would be chosen
    ...
    def prepare(self):
        ...
        self._set_model()
        self._cal_pivot() # do not affect the pruning
    ...
    def _set_dataloader(self):
        ...
        if 'cifar' in self.settings.dataset:
            ...
            self.train_loader, self.val_loader = get_sub_imagenet_dataloader(self.settings.dataset,
                                                                             ...
                                                                             self.settings.data_path,
                                                                             self.logger)
        elif self.settings.dataset in ['pcldriver']:
    ...
    def _set_channel_selection(self):
        # define the basic tool for pruning, this class contains many function for pruning
    ...
    def _set_model(self):
        ...
        # this line initializes a pruned_model the same as original one
        # so pruning is not happening here
    ...
    def _load_resume(self):
        ...
        if self.settings.resume is not None:
            print('program is not going into there!')
            time.sleep(10)
    ...
    def channel_selection_for_network(self):
        ...
        restart_index = None
        # find restart segment index, means start position of each segment, i.e., the first layer of each segment
        # pivot_set = [4, 8]
        ...
        for index in range(self.num_segments):# num_segments = 3, index in [0, 1, 2]
            ...
            for j in range(index + 1):
                ...
                # print('-----maybe problem existed at the very beginning')
                # print(self.segment_wise_trainer.pruned_segments[j])
                # time.sleep(10)
                # print()
            ...
            # print('-------------------------liuxu---debug-----------------------------')
            # print('original segment: \n{}'.format(original_segment))
            # print('pruned segment: \n{}'.format(pruned_segment))
            # print('--------------------------debug over-------------------------------')
        ...
        log_str = "cost time: {}".format(str(timedelta(seconds=time_interval)))
    ...
    def pruning(self):
        ...
        model_prune.run() # pruning happened, problem happened
        ...
        self.segment_wise_trainer.update_model(self.original_model, self.pruned_model) # pruning finished, update self.pruned_model
