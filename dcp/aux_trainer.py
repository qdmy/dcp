...
class View(nn.Module):
    ...
    def forward(self, x):
        assert x.dim() == 2 or x.dim() == 4, "invalid dimension of input {:d}".format(x.dim()) # x.dim()=2 means linear layer, =4 means conv layer
        if x.dim() == 4:
            out = x.view(x.size(0), -1) # change conv into 2 dimension, so that can prune
        else:
            out = x
        return out
...
class AuxTrainer(object):
    ...
    def __init__(self, model, train_loader, val_loader, settings, logger, tensorboard_logger, run_count=0):
        ...
        self.segments = []
        self.aux_fc = [] # add auxiliary loss, so does a fc layer to output predictions
        self.seg_optimizer = [] # every segment may use different optimizer
        self.fc_optimizer = []
    ...
    def set_loss_weight(self):
        ...
        base_weight = 0
        self.lr_weight = torch.zeros(len(self.settings.pivot_set)).cuda() # weight of learning rate
        self.pivot_weight = self.lr_weight.clone() # every auxiliary loss, give a initial weight 0
        num_layers = 1
        if self.settings.net_type == "preresnet" or \
            ...
            num_layers = 3
        for i in range(len(self.settings.pivot_set) - 1, -1, -1): # without final loss, backwards, calculate every auxiliary loss weight
    ...
    def create_segments(self):
        ...
        shallow_model = None # first define front layers as net_head before the first auxiliary loss show up
        ...
        if self.settings.net_type in ["resnet", "preresnet"]:
            for module in self.model.modules():
                if isinstance(module, (PreBasicBlock, Bottleneck, BasicBlock)):
                    ...
                    if block_count in self.settings.pivot_set: # final loss position is not in pivot_set
                        self.segments.append(shallow_model)
                        shallow_model = None

        self.final_block_count = block_count
        self.logger.info(self.final_block_count)
        self.segments.append(shallow_model) # add final loss layer into segments
        ...
        # shallow_model = None
        #
        # if self.settings.net_type in ["preresnet", "pruned_preresnet", "resnet", "pruned_preresnet"]:
        #     if self.settings.net_type in ["preresnet", "pruned_preresnet"]:
        #         shallow_model = nn.Sequential(self.model.conv)
        #     elif self.settings.net_type == "resnet":
        #         net_head = nn.Sequential(
        #             self.model.conv1,
        ...
        #             self.model.maxpool)
        #         shallow_model = nn.Sequential(net_head)
        # elif self.settings.net_type in ["vgg", "mobilenet", "mobilenetv1_imagenet"]:
        #     shallow_model = None
        # elif self.settings.net_type in ["mobilenetv2_imagenet"]:
        #     shallow_model = nn.Sequential(
        #         self.model.conv1,
        #         self.model.bn1,
        #         self.model.relu1)
        # elif self.settings.net_type in ["mobilenetv2_pytorch"]:
        #     shallow_model = nn.Sequential(
        #         self.model.features[0])
        # elif self.settings.net_type in ["mobilenetv2_cifar"]:
        #     shallow_model = nn.Sequential(
        #         self.model.conv1,
        #         self.model.bn1,
        #         nn.ReLU())
        ...
        #
        # block_count = 0
        # if self.settings.net_type in ["resnet", "preresnet", "mobilenetv2_imagenet",
        #                               "mobilenetv2_cifar", "mobilenetv2_pytorch", "pruned_preresnet"]:
        #     for module in self.model.modules():
        #         if isinstance(module, (PreBasicBlock, PrunedPreBasicBlock,
        #                                Bottleneck, BasicBlock,
        #                                MobileBottleneck, MobileNetV2CifarBlock,
        #                                InvertedResidual)):
        #             self.logger.info("enter block: {}".format(type(module)))
        #             if shallow_model is not None:
        #                 shallow_model.add_module(str(len(shallow_model)), module)
        ...
        #                 self.segments.append(shallow_model)
        #                 shallow_model = None
        # elif self.settings.net_type in ["vgg", "mobilenetv1_cifar", "mobilenetv1_imagenet"]:
        #     for module in self.model.modules():
        ...
        # self.final_block_count = block_count
        # self.logger.info(self.final_block_count)
        # # remove fc from shallow_model
        # if self.settings.include_fc and isinstance(utils.model2list(shallow_model)[-1], nn.Conv2d):
        #     shallow_model = nn.Sequential(*utils.model2list(shallow_model)[:-1])
        # self.segments.append(shallow_model)

    def create_auxiliary_classifiers(self):
        """
        We insert the auxiliary classifiers after the convolutional layer. # auxiliary classifier is fc layer
        """

        num_classes = self.settings.n_classes
        in_channels = 0
        for i in range(len(self.segments) - 1):
            if isinstance(self.segments[i][-1], (PreBasicBlock, BasicBlock)): # judge the type of last layer in each segment
    ...
    def create_optimizers(self):
        ...
        for i in range(len(self.segments)):
            ...
            for j in range(i + 1): # each layer index in this segment
                temp_optim.append({'params': self.segments[j].parameters(),
                                   'lr': self.settings.lr})
            # this is because every segment we seperate out, and every fc we add, can be seemed as a individual model
            # so need to define optimizer for every one of them
    ...
    # load new parameters into correspond position
    def update_aux_fc(self, aux_fc_state, aux_fc_opt_state=None, seg_opt_state=None):
        ...
        if len(self.aux_fc) == 1: # not add auxiliary loss and fc
    ...
    def auxnet_forward(self, images, labels=None):
        ...
        temp_input = images
        losses = []
        for i in range(len(self.segments)): # forward segment-wise
            # forward
    ...
    def auxnet_backward_for_loss_i(self, loss, i): # do BP for auxiliary loss or final loss
        """
        ...
        if i < len(self.seg_optimizer) - 1: # only update the layers inside this segment
            for param_group in self.seg_optimizer[i].param_groups:
                ...
                param_group['lr'] = self.lr * self.adjustweight(self.lr_weight[i].item()) # as defined, each lr_weight is a torch.tensor
    ...
    def update_lr(self, epoch):
        ...
        if hasattr(self.settings, 'warmup_n_epochs'):
            if epoch < self.settings.warmup_n_epochs:
                ...
                gamma = 0 # seems like by self defining the exp scheduler
